...
class WebSocketHandler(DirectIntentMixin):
    ...
    def _reset_transient_downstream_state(self) -> None:
        self.state.is_interrupted = False
        self.state.model_audio_last_seen_at = 0.0
        self.state.transcript_buffer = ""
        self.state.transcript_buffer_started_at = 0.0
    ...
    async def _retry_downstream_or_request_reconnect(
        self,
        *,
        reason: str,
        retry_ms: int,
        ...
        if (
            self.stop_downstream.is_set()
            or self.websocket.client_state != WebSocketState.CONNECTED
        ):
            return False

        if self.state.downstream_retry_count < self.state.DOWNSTREAM_MAX_RETRIES:
            self.state.downstream_retry_count += 1
            backoff_sec = compute_retry_backoff(self.state.downstream_retry_count)
            self._reset_transient_downstream_state()
            logger.warning(
                "%s; retrying downstream internally (%d/%d) after %.1fs: user=%s session=%s",
                reason,
        ...
        logger.warning(
            "%s persisted after %d retries; requesting client reconnect: user=%s session=%s",
            reason,
            self.state.downstream_retry_count,
            self.user_id,
            self.session_id,
        ...
        self.stop_downstream.set()
        try:
            await self.websocket.close(code=1012, reason=close_reason)
        except Exception:
            pass
        return False
    ...
    async def _downstream(self) -> None:
        ...
        try:
            ...
            if (
                not self.stop_downstream.is_set()
                and not go_away_requested
                and self.websocket.client_state == WebSocketState.CONNECTED
            ):
                should_retry = await self._retry_downstream_or_request_reconnect(
                    reason="Live event stream ended unexpectedly",
                    retry_ms=1500,
                    client_message="Live stream interrupted. Please reconnect.",
                    close_reason="live_stream_ended",
                )
                if should_retry:
            ...
            if not self._downstream_ready.is_set():
                ...
                self._downstream_ready.set()
            exc_text = flatten_exception_text(exc)
            if is_retryable_transport_error(exc_text):
                should_retry = await self._retry_downstream_or_request_reconnect(
                    reason=f"Transient Live API transport error ({exc_text[:120]})",
                    retry_ms=2000,
                    client_message="Live stream interrupted. Please reconnect.",
                    close_reason="live_transport_error",
                )
                if should_retry:
                    return await self._downstream()
                return
